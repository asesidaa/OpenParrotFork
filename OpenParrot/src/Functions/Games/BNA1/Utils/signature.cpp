// https://github.com/XLabsProject/iw6x-client/blob/develop/src/common/utils/signature.cpp
...
namespace utils::hook
{
	void signature::load_pattern(const std::string& pattern)
	{
		...
		for(auto val : pattern)
		{
			...
			else
			{
				...
				{
					has_nibble = false;
					const uint8_t byte = current_nibble |static_cast<uint8_t>(nibble << 4);
	...
	std::vector<size_t> signature::process_range_vectorized(uint8_t* start, const size_t length) const
	{
		...
		{
			desired_mask[i / 8] |= (this->mask_[i] == '?' ? 0 : 1) << i % 8;  // NOLINT(bugprone-narrowing-conversions, cppcoreguidelines-narrowing-conversions)
		}
	...
	signature::signature_result signature::process_serial() const
	{
		...
		return signature_result(this->process_range(this->start_, this->length_ - sub));
	}
	...
	signature::signature_result signature::process_parallel() const
	{
		...
		return signature_result(std::move(result));
	}
