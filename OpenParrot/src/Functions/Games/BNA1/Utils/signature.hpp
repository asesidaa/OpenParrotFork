// https://github.com/XLabsProject/iw6x-client/blob/develop/src/common/utils/signature.hpp
...
namespace utils::hook
{
    ...
    public:
        ...
        {
        public:
            explicit signature_result(std::vector<size_t>&& matches) : matches_(std::move(matches))
            {
            ...
            [[nodiscard]] uint8_t* get(const size_t index) const
            {
                ...
                return reinterpret_cast<uint8_t*>(this->matches_[index]);  // NOLINT(performance-no-int-to-ptr)
            }
