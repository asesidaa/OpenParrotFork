// https://github.com/XLabsProject/iw6x-client/blob/develop/src/common/utils/nt.cpp
#include "nt.hpp"
...
namespace utils::nt
{
		...
		while (import_descriptor->Name)
		{
			if (!_stricmp(reinterpret_cast<char*>(this->get_ptr() + import_descriptor->Name), module_name.data()))
			{
				...
				while (original_thunk_data->u1.AddressOfData)
				{
					...
					if (GetProcAddress(other_module.module_, reinterpret_cast<char*>(ordinal_number)) ==  // NOLINT(performance-no-int-to-ptr, clang-diagnostic-pedantic)
						target_function)
					{
	...
	}  // NOLINT(clang-diagnostic-invalid-noreturn)

	std::string load_resource(const int id)
	{
		auto* const res = FindResource(HMODULE(library()), MAKEINTRESOURCE(id), RT_RCDATA);
		if (!res) return {};

		auto* const handle = LoadResource(nullptr, res);
		if (!handle) return {};

		return {static_cast<LPSTR>(LockResource(handle)), SizeofResource(nullptr, res)};
	}
